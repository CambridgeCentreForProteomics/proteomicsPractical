...
3. What else might inhibiting polymerisation of microtubules do to the cells?
...
For the purposes of this practical, peptides which cannot be uniquely assigned to a single protein have been removed. Peptides which come from proteins in the [cRAP database](http://www.thegpm.org/crap/) of common proteomics contaminates, e.g keratin, have also been removed. 
...
9. What does if mean to say a peptide is 'assigned' to a protein. Could a peptide be assigned to multiple proteins?
...
Below, we obtain the sum of peptide abundances in each sample (column). Note these are relatively similar but M_2 is ~2-fold less tha M_1 or G1_1.
```{r}
...
1. Suppose you screened many patients for their levels of a single known disease-associated protein with a blood test, would you try and control the FWER or the FDR? Why?
2. Suppose you quantified all proteins in the blood of patients with a disease and compared to matched healthy patients. How would you decide what significant changes in abundance are clinically relevant?
***
